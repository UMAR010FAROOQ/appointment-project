...
def DashPage(request):
    ...
    # Patients Today: Count of confirmed appointments for today
    ...
    # Total Patients: Count of all upcoming appointments
    ...
    # Completed Appointments: Count of all appointments marked as completed
...
def InstructorAppointments(request):
    ...
    today_appointments = Appointment.objects.filter(
        instructor=instructor_profile, appointment_date=today_date
    ).order_by('-appointment_date')

    upcoming_appointments = Appointment.objects.filter(
        instructor=instructor_profile, appointment_date__gt=today_date
    ).order_by('appointment_date')

    completed_appointments = Appointment.objects.filter(
        instructor=instructor_profile, status='completed'
    ).order_by('-appointment_date')

    # Paginate each list
    today_paginator = Paginator(today_appointments, 1)  # 10 per page
    upcoming_paginator = Paginator(upcoming_appointments, 1)
    completed_paginator = Paginator(completed_appointments, 1)
    ...
    context = {
        'today_appointments': today_page_obj,
        'upcoming_appointments': upcoming_page_obj,
        'completed_appointments': completed_page_obj,
        'today_count': today_appointments.count(),
        'total_count': upcoming_appointments.count() + today_appointments.count(),
        'completed_count': completed_appointments.count(),
    }

    return render(request, 'instructors/instructor-appointments.html', context)
